...
use snow_draw_engine_document::{
    ElementId, TextData, TextLayoutSize, normalize_font_family, text_with_measured_layout,
...
pub(crate) fn text_with_committed_draft(
    ...
    // The draft's measured layout is authoritative for the committed element;
    // keeping the base ink box would leave serial connectors and dirty regions
    // anchored to the ink of the pre-edit text.
    updated = text_with_measured_layout(&updated, layout)?;
...
mod tests {
    ...
    use snow_draw_engine_document::{FillStyle, InkBox, TextHorizontalAlign, TextVerticalAlign};
    ...
    #[test]
    fn committed_draft_replaces_stale_ink_box_with_measured_layout() {
        // Regression: a serial-bound label is created with the ink box of the
        // empty draft; editing grows the text and the host re-measures. The
        // commit must persist the measured ink, otherwise serial connectors
        // keep anchoring to the pre-edit (under-calculated) text bounds.
        let draft = TextDraftCommit::new(
