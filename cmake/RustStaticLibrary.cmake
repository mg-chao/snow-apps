...
function(snow_add_rust_static_libraries batch_name)
    ...
    if(APPLE)
        list(APPEND _cargo_environment "MACOSX_DEPLOYMENT_TARGET=15.0")
    endif()
    if(_libclang_dir)
...
function(snow_add_rust_executable target_name)
    set(options PERFORMANCE_PROFILE NO_DEFAULT_FEATURES REPRODUCIBLE)
    set(oneValueArgs PACKAGE MANIFEST_DIR OUTPUT_NAME PROFILE)
    ...
    if(SNOW_RUST_PROFILE)
        # The executable declares its own cargo profile (for example the
        # size-optimized updater helper) for every non-Debug configuration.
        if(CMAKE_CONFIGURATION_TYPES)
            set(_profile "$<IF:$<CONFIG:Debug>,debug,${SNOW_RUST_PROFILE}>")
            set(_cargo_profile "$<IF:$<CONFIG:Debug>,dev,${SNOW_RUST_PROFILE}>")
        elseif(CMAKE_BUILD_TYPE STREQUAL "Debug")
            set(_profile debug)
            set(_cargo_profile dev)
        else()
            set(_profile "${SNOW_RUST_PROFILE}")
            set(_cargo_profile "${SNOW_RUST_PROFILE}")
        endif()
    elseif(SNOW_RUST_PERFORMANCE_PROFILE)
    ...
    if(MSVC)
        set(_rust_reproducible_flags)
        if(SNOW_RUST_REPRODUCIBLE)
            set(_rust_reproducible_flags " -C link-arg=/Brepro")
        endif()
        set(_snow_rust_static_crt FALSE)
        if(SNOW_APPS_RELEASE_STATIC OR SNOW_SHOT_RELEASE_STATIC OR
        ...
        if(_snow_rust_static_crt)
            list(INSERT _cargo_environment 0
                "RUSTFLAGS=$ENV{RUSTFLAGS} -Dwarnings -C target-feature=+crt-static${_rust_reproducible_flags}")
        elseif(SNOW_RUST_REPRODUCIBLE)
            list(INSERT _cargo_environment 0
                "RUSTFLAGS=$ENV{RUSTFLAGS} -Dwarnings${_rust_reproducible_flags}")
        endif()
    endif()
    set(_cargo_feature_args)
