...
fn main() -> Result<()> {
    ...
    let mut encode_push_ms = Vec::new();
    let mut frame_age_ms = Vec::new();
    let mut capture_stages: BTreeMap<&str, Vec<f64>> = BTreeMap::new();
            ...
            Ok(CaptureEvent::Frame(frame)) => {
                ...
                for stage in frame.metadata().stage_timings() {
                    capture_stages
                        .entry(stage.name)
                        .or_default()
                        .push(stage.duration.as_secs_f64() * 1000.0);
                }
                let timestamp_ms = captured_at.duration_since(measured_start).as_millis() as u64;
                let pts = (u128::from(timestamp_ms) * u128::from(fps) + 500) / 1000;
                let push_started = Instant::now();
                encoder
                    .as_mut()
                    .unwrap()
                    .push_rgba_frame(timestamp_ms, frame.as_rgba_bytes())?;
                let push_elapsed = push_started.elapsed().as_secs_f64() * 1000.0;
                push_ms.push(push_elapsed);
                if previous_pts.is_some_and(|previous| pts > previous) {
                    encode_push_ms.push(push_elapsed);
                }
                previous_pts = Some(previous_pts.map_or(pts, |previous: u128| previous.max(pts)));
    ...
    fs::write(
        ...
        format!(
            "encoded_frames,copy_ms,convert_ms,send_ms,drain_and_mux_ms,packet_receive_ms,mux_write_ms\n{},{:.3},{:.3},{:.3},{:.3},{:.3},{:.3}\n",
            ...
            timings.packet_receive.as_secs_f64() * 1000.0,
            timings.mux_write.as_secs_f64() * 1000.0
        ),
    )?;
    let mut stage_csv = String::from("stage,samples,total_ms,p50_ms,p95_ms\n");
    for (name, samples) in capture_stages {
    ...
    fs::write(output.with_extension("capture-stages.csv"), stage_csv)?;
    fs::write(
        output.with_extension("latency.csv"),
        format!(
            "capture_to_submit_p50_ms,capture_to_submit_p95_ms,encoding_push_p50_ms,encoding_push_p95_ms,encoded_per_wall_second\n{:.3},{:.3},{:.3},{:.3},{:.3}\n",
            percentile(&frame_age_ms, 0.5),
            percentile(&frame_age_ms, 0.95),
            percentile(&encode_push_ms, 0.5),
            percentile(&encode_push_ms, 0.95),
            report.encoded_frames as f64 / measured_seconds
        ),
    )?;
