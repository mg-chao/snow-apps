...
/* Events and rectangles are borrowed for the callback duration. Callbacks may run on
   either worker, are serialized, must copy/queue only, and must not re-enter the service.
   Destroy closes delivery synchronously but joins provider workers in the background. */
