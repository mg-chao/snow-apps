...
1. Create a service and initialize options with `snow_selected_text_options_init`. The default Auto
   strategy prefers Accessibility and then uses guarded Copy fallback. Snow Shot keeps Auto on
   macOS and explicitly selects clipboard-only capture on Windows.
...
two-second timeout. Explicit `STRATEGY_ACCESSIBILITY`, `STRATEGY_NATIVE_CONTROL`, and
`STRATEGY_CLIPBOARD` attempt only the chosen method; native-control acquisition is Windows-only and
`copy_fallback` affects only `STRATEGY_AUTO`. Unknown strategy values are rejected before reading
exclusion arrays. Rebuild source consumers for the generic Accessibility/native-control names;
numeric values, layout, and ABI version 1 are unchanged. Error operation strings have static
lifetime. No thread-local last-error state is used.
