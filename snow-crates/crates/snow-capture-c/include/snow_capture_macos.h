...
typedef enum SnowMacStatus {
    SNOW_MAC_OK = 0, SNOW_MAC_INVALID_ARGUMENT = 1, SNOW_MAC_PERMISSION_DENIED = 2,
    SNOW_MAC_TARGET_UNAVAILABLE = 3, SNOW_MAC_UNSUPPORTED = 4, SNOW_MAC_TIMEOUT = 5,
    SNOW_MAC_CANCELED = 6, SNOW_MAC_INTERRUPTED = 7, SNOW_MAC_INTERNAL = 255
} SnowMacStatus;
...
typedef struct SnowMacCaptureConfig {
    ...
    uint32_t target_kind; /* 0 primary display, 1 display ID, 2 session-scoped window ID, 3 desktop region */
    uint32_t target_id;
    ...
    double source_x, source_y, source_width, source_height; /* desktop points, top-left; region only */
...
SnowMacStatus snow_capture_macos_query_capabilities(const SnowMacCaptureConfig*, SnowMacCapabilities*);
...
SnowMacStatus snow_capture_macos_enumerate(uint32_t kind, uint32_t timeout_ms,
    void (*visitor)(const SnowMacTargetInfo*, void*), void* context);
...
SnowMacStatus snow_capture_macos_stream_next(SnowMacStream*, uint32_t timeout_ms, SnowMacCaptureEvent*);
...
SnowMacStatus snow_capture_macos_frame_source_time(const SnowMacFrame*, uint32_t index, SnowMacSourceTime*);
...
SnowMacStatus snow_capture_macos_frame_map_format(const SnowMacFrame*, uint32_t format, uint32_t plane, SnowMacPlane*);
SnowMacStatus snow_capture_macos_frame_map_plane(const SnowMacFrame*, uint32_t index, SnowMacPlane*);
