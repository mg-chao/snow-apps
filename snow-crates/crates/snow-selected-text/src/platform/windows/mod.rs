...
pub(crate) fn capture_source() -> Result<SourceApplication, SelectionError> {
    ...
    unsafe {
        ...
        Ok(SourceApplication {
            native_window: Some(window.0 as usize),
            process_id,
            native_focus: (!info.hwndFocus.is_invalid()).then_some(info.hwndFocus.0 as usize),
            executable,
        })
    }
}
...
pub(super) fn validate(context: &Context) -> Result<(), SelectionError> {
    context.check()?;
    unsafe {
        let window = hwnd(context.source.native_window.unwrap());
        ...
        GetGUIThreadInfo(thread, &mut info).map_err(|e| api_error("GetGUIThreadInfo", e))?;
        let native_focus = (!info.hwndFocus.is_invalid()).then_some(info.hwndFocus.0 as usize);
        if native_focus != context.source.native_focus {
            return Err(SelectionError::new(
                ErrorKind::TargetChanged,
                "native focus",
            ));
...
pub(super) fn check_copy_allowed(context: &Context) -> Result<(), SelectionError> {
    let native_class = class_name(hwnd(context.source.native_window.unwrap()));
    ...
    if let Some(focus) = context.source.native_focus {
        native::check_password(hwnd(focus))
    } else {
        Ok(())
    }
}
