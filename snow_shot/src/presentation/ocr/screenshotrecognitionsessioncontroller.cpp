...
void ScreenshotRecognitionSessionController::startTranslationWithModels(
    ...
    const auto selected =
        std::find_if(models.cbegin(), models.cend(), [&settings](const auto& model) {
            return model.supportsTranslation() && model.id == settings.modelId;
        });
    if (selected == models.cend()) {
        ...
        settings.modelId =
            general != models.cend() ? general->id : m_tableRecognition->fallbackModel(false);
...
void ScreenshotRecognitionSessionController::showTranslationSettingsModal(
    ...
    if (!models.isEmpty() &&
        std::none_of(models.cbegin(), models.cend(), [&current](const auto& model) {
            return model.supportsTranslation() && model.id == current.modelId;
        })) {
        const auto general = std::find_if(models.cbegin(), models.cend(), [](const auto& model) {
            return model.supportsTranslation() &&
                   model.translationMode == QStringLiteral("default");
    ...
    const auto applyModels = [form, service, errorAlert, modal,
        ...
        const bool currentAvailable =
            std::any_of(availableModels.cbegin(), availableModels.cend(),
                        [&selectedId](const SnowShotChatModel& model) {
                            return model.supportsTranslation() && model.id == selectedId;
                        });
        const auto general =
