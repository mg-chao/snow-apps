...
void ScreenshotController::requestIdleResourceRelease() {
    if (m_impl != nullptr) {
        scheduleIdleImplementationRelease(m_impl.get());
    }
}

void ScreenshotController::cancelIdleResourceRelease() {
    // Keep this token separate from operation generations. Reopening the application interface
    // must cancel a pending lifecycle teardown without invalidating an in-flight export or a
    // delayed-capture request.
    ++m_idleReleaseGeneration;
}
