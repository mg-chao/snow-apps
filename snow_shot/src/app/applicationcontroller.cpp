...
class ApplicationController::Impl {
  public:
    ...
    MainWindow& ensureMainWindow() {
        // A close-triggered idle release is deliberately deferred so queued widget destruction
        // and capture-worker callbacks can settle. Showing the interface again is an explicit
        // demand for the long-lived controller, so cancel that deferred teardown first.
        if (screenshotController != nullptr) {
            screenshotController->cancelIdleResourceRelease();
        }
            ...
            QObject::connect(
                ...
                [this, closedWindow = QPointer<MainWindow>(mainWindow.get())]() {
                    ...
                    QTimer::singleShot(0, &q, [this, closedWindow]() {
                        ...
                        mainWindow.reset();
                        if (screenshotController != nullptr) {
                            // MainWindow owns the page tree, while ScreenshotController owns the
                            // independent capture graph. Release the latter only after the window
                            // tree has been destroyed and the close event has returned.
                            screenshotController->requestIdleResourceRelease();
                        }
                    });
                });
